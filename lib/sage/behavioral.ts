...
export function buildBehavioralContext(behavior: BrowsingBehavior, cartTotal?: number, cartItems?: CartItemInfo[]): string {
    ...
    if (cartItems && cartItems.length > 0) {
        ...
        parts.push(`CART CONTENTS (${cartItems.length} item${cartItems.length !== 1 ? "s" : ""}, $${total.toFixed(2)} total):\n${itemList}\n${
            remaining > 0
                ? `Customer is $${remaining.toFixed(2)} away from free shipping ($150+ threshold).`
        ...
        const remaining = 150 - cartTotal;
        if (remaining > 0) {
            parts.push(`CART STATUS: Customer has $${cartTotal.toFixed(2)} in their cart. They are $${remaining.toFixed(2)} away from free shipping ($150+ threshold). If it comes up naturally, you can mention this — but never push products just to hit the threshold.`);
        } else {
